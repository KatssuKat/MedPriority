        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              St. Vincent's General · Emergency Check-In
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={s} className="flex items-center gap-3 flex-1">
                <div
                  className={`size-8 rounded-full flex items-center justify-center text-sm font-semibold ${done ? "bg-success text-primary-foreground" : active ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                >
                  {i + 1}
                </div>
                <div
                ...
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-1 rounded-full ${done ? "bg-success" : "bg-muted"}`} />
                )}
              </div>
            );
          })}
        ...
        <div className="w-full max-w-3xl">
          {step === 1 && (
            <div className="bg-surface rounded-3xl border border-border p-10 shadow-xl shadow-primary/5">
              ...
              <div className="mt-8 grid grid-cols-1 gap-4">
                ...
                <Field
                  label="Reason for visit (short)"
                  placeholder="Chest discomfort since this morning"
                />
              </div>
            </div>
          )}
            ...
            <div className="bg-surface rounded-3xl border border-border p-10 shadow-xl shadow-primary/5">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`h-16 rounded-2xl border text-base font-medium flex items-center justify-center gap-3 ${mode === "voice" ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted/40"}`}
                >
                  <Mic className="size-5" /> Use voice
                </button>
                <button
                  onClick={() => setMode("text")}
                  className={`h-16 rounded-2xl border text-base font-medium flex items-center justify-center gap-3 ${mode === "text" ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted/40"}`}
                >
              ...
              {mode === "voice" ? (
                <div className="mt-8 rounded-3xl bg-gradient-to-b from-primary/5 to-accent/30 border border-border p-10 flex flex-col items-center text-center">
                  ...
                  <div className="mt-2 text-muted-foreground">
                    "I have sharp chest pain and shortness of breath."
                  </div>
                </div>
              ) : (
                <textarea
                  className="mt-8 w-full h-44 rounded-2xl border border-border bg-muted/40 p-5 text-lg focus:outline-none focus:ring-2 focus:ring-ring"
              ...
              <div className="mt-6">
                <div className="text-sm text-muted-foreground mb-3">
                  Common symptoms — tap any that apply
                </div>
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      key={t}
                      className="h-12 px-5 rounded-full border border-border bg-surface hover:border-primary hover:bg-primary/5 text-sm font-medium"
                    >
                      {t}
                    </button>
                  ))}
          ...
          {step === 3 && (
            <div className="bg-surface rounded-3xl border border-border p-10 shadow-xl shadow-primary/5">
                ...
                {Array.from({ length: 11 }).map((_, i) => {
                  ...
                  const tone =
                    i <= 3
                      ...
                      : i <= 6
                        ? "bg-priority-medium/20 text-priority-medium border-priority-medium/40"
                        : i <= 8
                          ? "bg-priority-high/20 text-priority-high border-priority-high/40"
                          : "bg-priority-critical/20 text-priority-critical border-priority-critical/40";
                  return (
                    <button
                      key={i}
                      onClick={() => setPain(i)}
                      className={`flex-1 h-20 rounded-2xl border-2 text-2xl font-semibold transition-all ${active ? `${tone} scale-110 shadow-lg` : "border-border bg-muted/40 hover:bg-muted text-muted-foreground"}`}
                    >
                      {i}
                    </button>
                  );
              ...
              <div className="mt-10 rounded-2xl bg-muted/40 border border-border p-5 flex gap-4 items-center">
                <div
                  className={`size-14 rounded-2xl flex items-center justify-center text-2xl font-bold ${pain <= 3 ? "text-priority-low bg-priority-low/15" : pain <= 6 ? "text-priority-medium bg-priority-medium/15" : pain <= 8 ? "text-priority-high bg-priority-high/15" : "text-priority-critical bg-priority-critical/15"}`}
                >
                  {pain}
                </div>
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    A nurse will review your input shortly.
                  </div>
                </div>
              </div>
            </div>
          ...
          {step === 4 && (
            <div className="bg-surface rounded-3xl border border-border p-10 shadow-xl shadow-primary/5">
              ...
              <p className="text-muted-foreground mt-2 text-lg">
                Please take a seat. We've passed your information to the triage nurse.
              </p>
          ...
          <div className="mt-6 flex items-center justify-between">
            <button
              ...
              className="h-14 px-6 rounded-2xl border border-border bg-surface flex items-center gap-2 text-base font-medium disabled:opacity-40"
              disabled={step === 1}
            >
            ...
            <button
              ...
              className="h-14 px-8 rounded-2xl bg-primary text-primary-foreground flex items-center gap-2 text-base font-semibold shadow-lg shadow-primary/30"
            >
...
function Field({ label, placeholder }: { label: string; placeholder: string }) {
  return (
    <label className="block">
      ...
      <input
        className="mt-2 w-full h-14 px-4 rounded-2xl border border-border bg-muted/40 text-lg focus:outline-none focus:ring-2 focus:ring-ring"
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "medium" }) {
  return (
    <div
      className={`rounded-2xl p-5 border ${tone === "medium" ? "bg-priority-medium/15 border-priority-medium/40" : "bg-muted/40 border-border"}`}
    >
