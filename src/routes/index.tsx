...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "AI-assisted triage, ER monitoring and analytics for modern hospitals.",
      },
    ],
  }),
});
...
const screens = [
  {
    to: "/kiosk",
    title: "Patient Kiosk",
    desc: "Self check-in, voice symptoms, accessible flow.",
    icon: Tablet,
    tone: "from-info/20 to-primary/10",
  },
  {
    to: "/triage",
    title: "Clinical Triage",
    desc: "AI-suggested ESI level with vitals & history.",
    icon: Stethoscope,
    tone: "from-accent to-primary/10",
  },
  {
    to: "/er",
    title: "ER Dashboard",
    desc: "Live patient board, severity sorting, alerts.",
    icon: Activity,
    tone: "from-priority-high/15 to-priority-critical/10",
  },
  {
    to: "/admin",
    title: "Analytics",
    desc: "Saturation, KPIs, downloadable reports.",
    icon: BarChart3,
    tone: "from-success/15 to-info/10",
  },
];
        ...
        <section className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-surface to-accent p-8">
          ...
          <div className="relative max-w-2xl">
            ...
            <h1 className="mt-4 text-4xl font-semibold tracking-tight">
              Triage decisions, accelerated by clinical AI.
            </h1>
            <p className="mt-3 text-muted-foreground max-w-xl">
              MedPriority unifies patient intake, ESI classification, and ER orchestration into one
              calm, defensible workflow — so the right patient is always seen first.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                to="/triage"
                className="inline-flex items-center gap-2 px-4 h-11 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-95"
              >
          ...
          {stats.map((s) => (
            <div key={s.label} className="glass rounded-xl p-5">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                ...
                <div className={`text-xs ${s.danger ? "text-destructive" : "text-success"}`}>
                  {s.delta}
                </div>
              </div>
            </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {screens.map((s) => (
            <Link
              key={s.to}
              to={s.to}
              className={`group relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br ${s.tone} p-6 hover:border-primary/40 transition-colors`}
            >
